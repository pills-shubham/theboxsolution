        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
                ...
                <Card className="mb-6">
                  <CardContent className="pt-6">
                    <form onSubmit={handleAddAddress} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <Button className="md:col-span-2" type="submit">Save Address</Button>
                    </form>
                  </CardContent>
                </Card>
              )}
                ...
                {addresses.map((addr) => (
                  ...
                  >
                    <div className="flex justify-between items-start">
                      ...
                      {selectedAddressId === addr.id && <Check className="size-5 text-primary" />}
                    </div>
                  </div>
                ))}
              </div>
            ...
            <section>
                ...
                <div className="space-y-2">
                    {items.map(item => (
                        <div key={item.id} className="flex justify-between items-center text-sm p-3 bg-muted/50 rounded-lg">
                            <span>{item.product?.title} x {item.quantity}</span>
                            <span className="font-medium">${((item.product?.price || 0) * (1 - (item.product?.discount_percent || 0) / 100) * item.quantity).toFixed(2)}</span>
                        </div>
                    ))}
                </div>
            </section>
