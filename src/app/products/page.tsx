    ...
    <div className="min-h-screen bg-background pt-20 sm:pt-24 pb-12">
      ...
      <main className="container mx-auto px-4">
        <div className="mb-8 sm:mb-12">
          <h1 className="text-2xl sm:text-4xl font-extrabold tracking-tight mb-2 sm:mb-4">Our Packaging Solutions</h1>
          <p className="text-muted-foreground text-sm sm:text-lg">Premium boxes and mailers for every need.</p>
        </div>

        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="h-[350px] sm:h-[400px] bg-muted rounded-xl animate-pulse" />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
